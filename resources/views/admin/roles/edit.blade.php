...
<div class="container-fluid py-3">
    ...
    <div class="mb-3">
        ...
        <h4 class="mb-1 text-dark fw-bold"><i class="bi bi-pencil-square me-2 text-primary"></i>Edit Role: {{ $role->name }}</h4>
        <p class="text-muted small">Update role details and status configurations.</p>
    </div>
            ...
            <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control form-control-sm @error('name') is-invalid @enderror" 
                           placeholder="E.g., Manager, Finance, Support" value="{{ old('name', $role->name) }}" required>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" class="form-control form-control-sm @error('description') is-invalid @enderror" 
                              rows="4" placeholder="Briefly describe the responsibilities associated with this role...">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
